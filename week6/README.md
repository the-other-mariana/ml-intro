...
If we have **X** and **y**, we will generate a vector **h** with the hypotheses for each trainind data value (rows of **X**, or vector **x_i**), which are like approxiamtions for **y**.
...
Only by changing **x_0** vector. In this way, we are updating each value of **a**. At the end, **a** will have the values a_0 ... a_n that minimize the J(**a**) function.
