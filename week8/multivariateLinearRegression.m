...
% mean(matrix) will compute the mean by column and will return a vector of n means for a matrix of n columns
mean_x = mean(x);
...
input_data = [3100 4];
norm_input = (input_data - mean_x)./ sigma;
norm_input = [1 norm_input];

h_input = a' * norm_input';
figure(3);
plot3(norm_input(2), norm_input(3), h_input, 'ok', 'MarkerFaceColor', 'm', 'MarkerSize', 8);

display(["The J Error: ", num2str(J)])
display(["Total Iterations: ", num2str(iter)])
display(["Input Hypothesis: ", num2str(h_input)])
